...
export const handler = async (event) => {
    try {
        ...
        const cachedWeatherData = await getCachedWeatherData(longitude, latitude);
        ...
        const openWeatherAPIURL = "https://api.openweathermap.org/data/2.5/onecall?lat=" + latitude + "&lon=" + longitude + "&units=" + unit + "&appid=" + response.SecretString;
        ...
        await cacheWeatherData(longitude, latitude, weatherData);
...
async function getCachedWeatherData(longitude, latitude) {
    const params = {
        TableName: process.env.CFDynamoDBWeatherTableName,
        Key: {
            "location": { S: longitude.toString() + ',' + latitude.toString() }
        }
    };
...
async function cacheWeatherData(longitude, latitude, weatherData) {
    const params = {
        TableName: process.env.CFDynamoDBWeatherTableName, 
        Item: {
            "location": { S: longitude.toString() + ',' + latitude.toString() },
